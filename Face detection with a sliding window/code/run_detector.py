            ...
            for row in range(hog_feature.shape[0]-cell_num+1):
                for col in range(hog_feature.shape[1]-cell_num+1):
                    ...
                    if tmp_confidences[0,0] > -0.2:
                        ...
                        tmp_bboxes = np.array([[cur_x_min, cur_y_min, cur_x_max, cur_y_max]])
                        cur_bboxes = np.concatenate([cur_bboxes, tmp_bboxes], axis=0)
                        cur_image_ids = np.concatenate([cur_image_ids, [[test_images[num]]]], axis=0)
